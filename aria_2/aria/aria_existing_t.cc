...
#define SKEW_PLACE 0
#define MAX_OPE_FOR_LONG 2000
#define RW_RATE_LONG 25
...
//concurrency control phase
    ...
    //execution phase
        ...
        sync_point.arrive_and_wait();
        if(thread_id == 0 && abort_num <= 60){
            re_ratio = (double)re_num/(double)THREAD_NUM;
            waw_ratio = (double)waw_num/(double)THREAD_NUM;
            cout << batch_id << " " << tid << " " << abort_num << " " << "waw" << waw_ratio << "re" << re_ratio << std::endl;
        }
        if(thread_id == 0){
            abo += abort_num;
            abort_num = 0;
    ...
    if(thread_id == 0){
        cout << (double)abo/(double)(batch_id-1) << std::endl;
    }
