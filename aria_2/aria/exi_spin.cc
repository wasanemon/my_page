        ...
        for(int i = 0;i < TX_PAR_THREAD;i++){
            if (transactions[i].status_ != Status::ABORTED)
            {
                ...
                //{
                    ...
                    //goto POINT;
                //}
                transactions[i].tx_pos_ = __atomic_load_n(&tx_counter, __ATOMIC_SEQ_CST);
                if (transactions[i].tx_pos_ >= PRE_NUM)
                {
                    return;
                }
                __atomic_store_n(&tx_counter, transactions[i].tx_pos_ + 1, __ATOMIC_SEQ_CST);
